...
describe('TodoForm', () => {
  test('새로운 투두 추가 테스트', async () => {
    ...
    (useCreateTodoMutation as jest.Mock).mockReturnValue({ mutate: mockMutate });
    ...
    expect(mockMutate).toHaveBeenCalledWith({ state: 'TODO', content: '새로운 할 일' });
  ...
  test('완료되지 않은 할 일이 10개 이상일 때 알림 메시지를 표시하는지 테스트', async () => {
    ...
    // alert 호출 여부 확인
    expect(alertMock).toHaveBeenCalledWith('아직 완료되지 않은 할 일이 10개 이상이에요!');
